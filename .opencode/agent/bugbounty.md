...
4. **Priority, hypotheses, proof**: generate baseline hypotheses from imported identities, workflow sensitivity, coverage, and program duplicate risk. For a specific business-logic idea, save it with `hypothesis add` using the actual actor, owner boundary, state, and statement rather than forcing it into a generic URL heuristic. Passive analysis may proceed; any request replay, mutation, scan, or validation requires the applicable approval.
5. **Attachments and learning**: preserve evidence, update hypothesis status, record duplicates/informative outcomes and submissions, and close or pause the hunt session with counts and notes.

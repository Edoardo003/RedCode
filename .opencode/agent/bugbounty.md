...
2. **Architecture, actors, assets**: model approved identities, tenants, roles, sensitive objects, trust boundaries, workflows, and lifecycle states. Import can correlate path/query/request/response identifiers using redacted HMAC fingerprints and enrich endpoint display templates, but role candidates remain `unknown`/`proposed`/`inferred` until an analyst confirms them with `identifier confirm`. Use `identifier relationship list/confirm` to review co-occurring role relationships; a lead is not a confirmed business fact. After the structural map, ask only the smallest business-context questions that change a test decision and persist the answers with `workflow annotate`, `workflow state set`, `workflow transition add`, and `workflow invariant add`. Never infer an owner, tenant, role, or lifecycle state from a URL alone, and never persist live session secrets or raw identifier values in mapping tables.
